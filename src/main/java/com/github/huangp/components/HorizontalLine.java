...
public class HorizontalLine implements Drawable {
    ...
    public Canvas draw(Canvas canvas) {
        ...
        if (rowNum < canvas.height()) {
            ...
            if (canvas.isColumnDrawable(colStartNum) && canvas.isColumnDrawable(colEndNum)) {
                ...
                log.warn("invalid drawing instruction: column start and end are not within the canvas boundary");
            }
        }
        log.warn("invalid drawing instruction: row number is not within the canvas boundary");
        return canvas;
    }
}
